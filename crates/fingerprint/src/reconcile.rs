...
pub fn partition_roots(records: &[BTreeMap<String, String>], partitions: usize) -> Vec<String> {
    ...
    for record in records {
        ...
        let bucket = u64::from_str_radix(&fingerprint_bytes(key.as_bytes())[..8], 16).unwrap_or(0)
            as usize
            % count;
...
pub fn differing_partitions(
    ...
    left_roots
        .iter()
        .zip(right_roots.iter())
        .enumerate()
        .filter_map(|(index, (left_root, right_root))| (left_root != right_root).then_some(index))
        .collect()
}
